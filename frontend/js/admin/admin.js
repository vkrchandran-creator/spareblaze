...
function renderContact() {
    ...
    if (!el('contact-pageTitle')) return;
...
async function saveProdModal() {
    ...
    const body   = {
        ...
        images: finalImages, partNumbers, compatibleVehicles,
...
document.addEventListener('DOMContentLoaded', () => {
    const dropzone = document.getElementById('db-prod-image-dropzone');
    const fileInput = document.getElementById('db-prod-file-upload');
    const preview = document.getElementById('db-prod-image-preview');
    const hiddenInput = document.getElementById('db-prod-images');
    const token = localStorage.getItem('sb_admin_token') || sessionStorage.getItem('sb_admin_token');
    ...
    async function handleFiles(files) {
        ...
        try {
            ...
            if (json.success && json.data.urls) {
                let currentImages = [];
                try { currentImages = JSON.parse(hiddenInput.value); } catch(e){}
                currentImages = currentImages.concat(json.data.urls);
                hiddenInput.value = JSON.stringify(currentImages);
                
                // Update preview
                preview.innerHTML = currentImages.map(img => `<img src="${img}" style="height:40px;width:40px;object-fit:cover;border-radius:4px;border:1px solid var(--border)">`).join('');
            ...
            showToast('Upload failed. Start the backend server.');
        } finally {
